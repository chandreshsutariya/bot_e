...
def run_paymentlink_agent(data: PromptRequest):  
    try:
        with open('files/oppiwallet_userpaymentlink.json','r') as f:
            schema = json.load(f)
            ...
            def sanitize_profile_qr(obj):
                ...
                if isinstance(obj, dict):
                    for key in list(obj.keys()):
                        ...
                        # if key=="appId" and obj[key]!=None:
                        #     obj[key] = None
                        if key in ["adminCommissionAmount", "adminAssetAddress", "privateKey", "encryptedKey"]:
                                obj[key] = None
                        else:
