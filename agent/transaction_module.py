...
def run_transaction_agent(data: PromptRequest):  
    try:
        with open('files/oppiwallet_transaction.json','r') as f:
            schema = json.load(f)
        ...
        # 4. Scope all queries strictly to this user ID: ObjectId("{User_Id}")
        ...
        # Syntax Requirements:
        # -Valid MongoDB BSON structure, Do not add any extra characters.

        # Security Rules:
        # - Only use tables and columns explicitly listed in the Database Schema section.
        # - Do NOT infer or guess missing columns.
        # -Do not alter data types, naming, or structural details from the schema.
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                ...
                - Do not use "toUserId" in query as you don't have this data, you just have "fromUserId" that is {userid}.
        ...
        replacements = {
            ...
            'fromUserId': f"ObjectId:{userid}",
            'fromWalletId':f"ObjectId:{userid}",
            # 'toUserId' : f"ObjectId:{userid}",
            'User_id': f"ObjectId:{userid}",
        }
            ...
            def sanitize_profile_qr(obj):
                ...
                if isinstance(obj, dict):
                    for key in list(obj.keys()):
                        # TO remove non-disclosable keywords to remove from jso
                        if key in ["txHash", "uuid"]:
                            obj[key] = None
                        else:
