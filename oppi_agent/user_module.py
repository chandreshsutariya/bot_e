...
def run_user_agent(data: PromptRequest):  
    try:
        # print("==================", data.credentials)
        with open('files/oppiwallet_user.json','r') as f:
            schema = json.load(f)
            ...
            def sanitize_profile_qr(obj):
                ...
                if isinstance(obj, dict):
                    for key in list(obj.keys()):
                        ...
                        if key in ["privateKey", "googleAuthKey", "otp", "mobileOtp", "whatsAppOtp", "googleAuthOtp", "telegramOtp", "hasEncyptionWordKey", "encyptionWordKey", "secret", "apiKey", "googleAuthSecret"," privateKey", "publicKey", "registeredIpAddress"]:
                            obj[key] = None
                        else:
